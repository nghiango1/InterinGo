...
def buildFileModeCommand(input: str, *args, **kwargs) -> str:
...
def recheckOutFile(outputFilePath: str, result: bytes):
...
def writeOutFile(outputFilePath: str, result: bytes):
...
def generateOuputFiles():
    testFiles = getInputFileList()
    for fn in testFiles:
        if DEBUG:
            print(f"Genereting {fn} test file output ...")
